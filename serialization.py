...
def generate_mock_galaxy(size):
    galaxy = Galaxy.Galaxy()

    for i in range(0, size):
        galaxy.generate_object()

    return galaxy
def serialize(galaxy):
    # builder = flatbuffers.Builder(1024)
    builder = flatbuffers.Builder(0)

    # Serialize space objects
    space_objects = []
    for spaceObject in galaxy.objects:
...
def visualize_galaxy(galaxy):
    print(f"number of objects: {galaxy.num_objects} ")
    # Reconstruct surface points
    import matplotlib.pyplot as plt
    fig = plt.figure()
    ax = fig.add_subplot(projection='3d')
    for i in range(0, galaxy.num_objects):
        ax.scatter(galaxy.objects[i].x, galaxy.objects[i].y, galaxy.objects[i].z)
    plt.show()
